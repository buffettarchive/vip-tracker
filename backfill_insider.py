"""
backfill_insider.py  (코스닥 내부자 장내/장외 매수 · 초기화 후 재구축)
─────────────────────────────────────────────────────────────
기존 insider.json을 무시하고, 최근 BACKFILL_DAYS일치를 v2 방식
(본문 확인 → 장내/장외 매수만)으로 처음부터 다시 채운다.
Render에서 Command를 'python backfill_insider.py'로 잠깐 바꿔 1회 실행.
"""
...
def gh_put(payload, sha):
    ...
    body = {
        "message": f"insider: backfill rebuild {payload['updated_at']}",
...
def main():
    _, sha = gh_get()        # sha만 필요(덮어쓰기), 기존 데이터는 무시
    existing = {}
    last_seen = ""           # 처음부터 다시
    print("[info] 기존 내부자 데이터 무시하고 새로 구축")
