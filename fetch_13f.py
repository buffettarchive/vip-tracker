...
─────────────────────────────────────────────────────────────
미국 SEC EDGAR API를 호출하여 거장들의 최신 13F-HR(분기별 포트폴리오) 정보를 파싱합니다.
매일 1회 실행하여 분기별 신규 공시가 올라오면 us_vips.json을 갱신합니다.
"""
...
# SEC API는 User-Agent에 반드시 이메일을 포함할 것을 요구합니다 (정책)
s = requests.Session()
...
GURUS = {
    ...
    "파브라이 인베스트먼츠": "0001548760", # Dalal Street LLC
    "아크레 캐피탈 (척 아크레)": "0001158172"
}

# 주요 종목 한글 번역 사전 (여기에 없는 종목은 영문으로 노출됨)
TRANSLATE = {
    "APPLE INC": "애플 (AAPL)",
...
def clean_issuer_name(name):
    ...
    # TRANSLATE 사전에 있으면 한글로, 없으면 영문 반환
...
def get_latest_13f_xml_url(cik):
    ...
    for i, form in enumerate(forms):
        if form == "13F-HR":
            ...
            # 2. 해당 공시의 폴더 인덱스 조회 (xml 문서 위치 찾기)
...
def parse_13f_xml(xml_content):
    ...
    xml_text = re.sub(r'\sxmlns="[^"]+"', '', xml_content.decode('utf-8', errors='ignore'), count=1)
    ...
    for info in root.findall('.//infoTable'):
        ...
        # SEC value는 '천 달러' 단위이므로 1000을 곱함
        val = int(info.findtext('value') or 0) * 1000
...
def gh_put_file(data, sha):
    ...
    body = {
        "message": f"us_vips: {dt.datetime.now(dt.UTC).strftime('%Y-%m-%dT%H:%M:%SZ')}",
    ...
    for guru_name, cik in GURUS.items():
        ...
        if xml_url:
            ...
            if r.status_code == 200:
                ...
                print(f"  → 성공: {len(holdings)}개 종목 확인 완료 (보고일: {report_date})")
            time.sleep(1) # SEC 서버 부하 방지
